...
contract ProofOfDonationNFT1 is ERC1155, Ownable {
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, string[] memory newURIs, bytes memory data) 
        public onlyOwner {
        require(to != address(0), "ProofOfDonationNFT1: mint to the zero address");
        require(ids.length == amounts.length, "ProofOfDonationNFT1: ids and amounts length mismatch");
        require(ids.length == newURIs.length, "ProofOfDonationNFT1: ids and URIs length mismatch");
    ...
    function setTokenURI(uint256 tokenId, string memory newURI) public onlyOwner {
        require(bytes(newURI).length > 0, "ProofOfDonationNFT1: URI should be set");
